...
swabvid(pvid)
	struct vid *pvid;
{
	bswap32(pvid->vid_oss);
	bswap16(pvid->vid_osl);
	bswap16(pvid->vid_osa_u);
	bswap16(pvid->vid_osa_l);
	bswap32(pvid->vid_cas);
}
	...
	struct cfg *pcfg;
{
	...
	bswap16(pcfg->cfg_atm);
	bswap16(pcfg->cfg_prm);
	bswap16(pcfg->cfg_atm);
	bswap16(pcfg->cfg_rec);
	bswap16(pcfg->cfg_trk);
	bswap16(pcfg->cfg_psm);
	bswap16(pcfg->cfg_shd);
	bswap16(pcfg->cfg_pcom);
	bswap16(pcfg->cfg_rwcc);
	bswap16(pcfg->cfg_ecc);
	bswap16(pcfg->cfg_eatm);
	bswap16(pcfg->cfg_eprm);
	bswap16(pcfg->cfg_eatw);
	bswap16(pcfg->cfg_rsvc1);
	bswap16(pcfg->cfg_rsvc2);
}
